...
function table_export() {
    // var value = $("#s71200_network_xls_config").edatagrid('getRows');
    var obj = get_select();
...
$(function() {
    $("#s71200_network_xls_config").edatagrid({
        ...
        onClickRow: function(index, row) {
            $("#s71200_network_xls_config").edatagrid('endEdit', index);
        },
        onDestroy: function(index, row) {
            ...
            if (result == 'true') {
                ...
                $("#s71200_network_xls_config").datagrid('reload');
            }
        },
        //保存后执行
        onEndEdit: function(index, row, changs) {
            var old_data = $('#s71200_network_xls_config').datagrid('getEditor', { index: index, field: "tagCode" });
            var obj = get_select();
            status = false;
            ...
            if (row['rate'] > 0) {
                if (status == 'true') {
                    objectType = 'AV'
                    if (['S', 'Q', 'T', 'C', 'SM'].indexOf(row['readType']) != -1) {
                        objectType = 'AI'
                    }
                    if (!row.isNewRecord) { // 判断是否是更新数据
                        sql = "update PLC set tagCode='{0}',description='{1}',readType='{2}',address='{3}',dataType='{4}',rate='{5}',objectType='{6}' where deviceCode='{7}' and tagCode='{8}'".format(row['tagCode'], row['description'], row['readType'], row['address'], row['dataType'], row['rate'], objectType, obj.select.text, old_data['oldHtml']);
                    } else {
                        sql = "insert into PLC values ('{0}','{1}','{2}','{3}','{4}','{5}','{6}','{7}')".format(obj.select.text, row['tagCode'], row['description'], row['readType'], row['address'], row['dataType'], row['rate'], objectType)
                    }
        ...
        //     $('#s71200_network_xls_config').edatagrid('unselectAll')
        // },
        idField: 'tagCode',
        columns: [
            [{
                ...
                },
                {
                    ...
                    editor: {
                        ...
                        options: {
                            panelHeight: 'auto',
                            method: 'get',
                            url: "../static/json/s71200Type.json",
                            editable: false,
                            valueField: "text",
                            textField: "text",
                            required: true,
                                    ...
                                    $("#dv_area_div").dialog({
                                        ...
                                        buttons: [{
                                                ...
                                                handler: function() { // 新建工程
                                                    area_address = $('#area_address').textbox('getValue');
                                                    var row = $('#s71200_network_xls_config').datagrid('getSelected');
                                                    if (row != null) {
                                                        var index = $('#s71200_network_xls_config').datagrid('getRowIndex', row);
                                                        var smEditor2 = $('#s71200_network_xls_config').datagrid('getEditor', {
                                                            index: index,
                                                            field: 'readType'
                                                        });
                                                        $(smEditor2.target).combobox('setValue', 'DB' + area_address);
                                                        $("#dv_area_div").dialog('close');
                                                    }
                                                }
                                            },
                                            {
                                                ...
                                                handler: function() {
                                                    ...
                                                    if (row != null) {
                                                        ...
                                                        $(smEditor2.target).combobox('setValue', 'DB1');
                                                        $("#dv_area_div").dialog('close');
                                                    }
                                                }
                                            }
        ...
        toolbar: [{
            ...
            iconCls: 'icon-add',
            handler: function() {
                $('#s71200_network_xls_config').edatagrid('saveRow');
                var obj = get_select();
                ...
                $('#s71200_network_xls_config').edatagrid('addRow', {
                    row: {
                        tagCode: tagCode,
                        rate: 1.00
                    }
            ...
            iconCls: 'icon-redo',
            handler: function() {
                $('#s71200_network_xls_config').edatagrid('cancelRow');
            }
        }, '-', {
            text: self.parent.messages[initial]['common']['remove'],
            iconCls: 'icon-remove',
            handler: function() {
                $("#s71200_network_xls_config").edatagrid('destroyRow');
            }
        }, '-', {
                        ...
                        $("#templete_dialog").dialog({
                            ...
                            buttons: [{
                                    ...
                                    handler: function() {
                                        ...
                                        if (result == "true") {
                                            $("#s71200_network_xls_config").edatagrid('reload');
            ...
            iconCls: 'icon-template',
            handler: function() {
                // values = $("#s71200_network_xls_config").datagrid('getRows');
