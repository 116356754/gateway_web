...
function table_export() {
    // var value = $("#BacnetIP_xls_config").edatagrid('getRows');
    var obj = get_select();
    var sql = "select * from Bacnet where deviceCode='{0}'".format(obj.select.text);
    content = selectsql(obj.path, sql);
    value = JSON.parse(content);
    if (value.length == 0) {
        value = [{ 'tagCode': '', 'description': '', 'objectType': '', 'objectInstance': '' }]
    }
    json = JSON.stringify({ 'BaCnet': value })
    jsontoexcel(json, 'BacnetIP_config.xls')
}

$(function() {
    var q = 0;
    $("#BacnetIP_xls_config").edatagrid({
        ...
        onClickRow: function(index, row) {
            $("#BacnetIP_xls_config").edatagrid('endEdit', index);
        },
        onDestroy: function(index, row) {
            ...
            if (result == 'true') {
                ...
                $("#BacnetIP_xls_config").datagrid('reload');
            }
        },
        //保存后执行
        onEndEdit: function(index, row, changs) {
            var old_data = $('#BacnetIP_xls_config').datagrid('getEditor', { index: index, field: "tagCode" });
            var obj = get_select();
        ...
        columns: [
            [{
                ...
                {
                    field: 'objectType',
                    title: self.parent.messages[initial]['BaCnetIP']['objectType'],
                    width: 100,
                    align: 'center',
                    editor: {
                        type: 'combobox',
                ...
                },
                {
                    ...
                    title: self.parent.messages[initial]['BaCnetIP']['objectInstance'],
                    width: 100,
                    align: 'center',
                    editor: {
                        type: 'numberbox',
                ...
                },
                {
                    ...
                    title: self.parent.messages[initial]['BaCnetIP']['propertyPriority'],
                    width: 100,
                    align: 'center',
                    editor: {
                        type: 'combobox',
        ...
        toolbar: [{
            ...
            iconCls: 'icon-add',
            handler: function() {
                $('#BacnetIP_xls_config').edatagrid('saveRow');
                var obj = get_select();
                ...
                $('#BacnetIP_xls_config').edatagrid('addRow', {
                    row: {
                        tagCode: tagCode
                    }
                });
            ...
            iconCls: 'icon-redo',
            handler: function() {
                $('#BacnetIP_xls_config').edatagrid('cancelRow');
            }
        }, '-', {
            text: self.parent.messages[initial]['common']['remove'],
            iconCls: 'icon-remove',
            handler: function() {
                $("#BacnetIP_xls_config").edatagrid('destroyRow');
            }
        }, '-', {
            ...
            handler: function() {
                ...
                if (obj.index == 0) {
                    ...
                    if (template_list.length == 0) {
                        $.messager.alert(self.parent.messages[initial]['common']['system_hint'], self.parent.messages[initial]['BaCnetIP']['no_template'], "info")
                    } else {
